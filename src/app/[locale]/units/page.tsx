      ...
      <section className="py-16">
        <Container>
                ...
                <article className="grid gap-6 rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-paper)] p-6 transition-shadow hover:shadow-md sm:p-8 lg:grid-cols-[1.4fr_1fr]">
                  <div>
                    ...
                    <p className="mt-1 text-[var(--color-muted)]">
                      {u.bedrooms} {t("bedroom")} · {u.bathrooms} {t("bathroom")} · 1 {t("balcony")} · {num(u.sqft)} sq ft
                    </p>
                    <p className="mt-4 leading-relaxed text-[var(--color-ink-soft)]">{ud(`${u.code}pos`)}</p>
                    <p className="mt-2 text-sm leading-relaxed text-[var(--color-muted)]">
                      <strong className="text-[var(--color-ink)]">{t("bestFor")}</strong> {ud(`${u.code}best`)}
                    </p>
          ...
          <p className="mt-8 text-center text-sm text-[var(--color-muted)]">
            {t("disclaimer")} {c("completion")} {project.completionYear}.
          </p>
        </Container>
      </section>
    </>
